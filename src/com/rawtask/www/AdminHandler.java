...
public class AdminHandler extends AbstractHandler {
    ...
    public AdminHandler(TaskServer server) {
        this.server = server;
    }
    ...
    public void handle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (!target.equals("/rawtask/")) {
            response.sendRedirect("/rawtask/");
            return;
        }
        ...
        try {
            ...
            root.put("components", f.fetchRecordFromTable("component"));
            root.put("milestones", f.fetchRecordFromTable("milestone"));
            root.put("versions", f.fetchRecordFromTable("version"));
        } catch (final SQLException e1) {
            TaskServer.LOGGER.log(Level.SEVERE, "cannot fetch data", e1);
        }
        /* Get the template (uses cache internally) */
        final Template temp = cfg.getTemplate("admin.html");
